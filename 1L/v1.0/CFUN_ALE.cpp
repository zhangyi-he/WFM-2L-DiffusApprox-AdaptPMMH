...
// [[Rcpp::export]]
arma::drowvec simulateWFM_arma(const arma::dmat& fts_mat, const int& pop_siz, const double& int_frq, const int& int_gen, const int& lst_gen) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword k = 1; k < arma::uword(lst_gen - int_gen) + 1; k++) {
    ...
    ale_frq(0) = R::rbinom(2 * pop_siz, prob(0)) / 2 / pop_siz;
...
// [[Rcpp::export]]
arma::drowvec simulateWFD_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const double& int_frq, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
// [[Rcpp::export]]
List runBPF_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::drowvec path = simulateWFD_arma(sel_cof, dom_par, pop_siz, ale_frq_tmp(i), smp_gen(k - 1), smp_gen(k), ptn_num);
...
// [[Rcpp::export]]
double calculateLogLikelihood_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::drowvec path = simulateWFD_arma(sel_cof, dom_par, pop_siz, ale_frq_pst(i), smp_gen(k - 1), smp_gen(k), ptn_num);
...
// [[Rcpp::export]]
List calculateOptimalParticleNum_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& gap_num) {
  // ensure RNG gets set/reset
  RNGScope scope;

  arma::drowvec log_lik(300);
  for (arma::uword i = 0; i < 300; i++) {
    log_lik(i) = calculateLogLikelihood_arma(sel_cof, dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num);
  }
  ...
  if (log_lik_sdv(0) > 1.7) {
    while (log_lik_sdv(0) > 1.0) {
      ...
      for (arma::uword i = 0; i < 300; i++) {
        log_lik(i) = calculateLogLikelihood_arma(sel_cof, dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, opt_pcl_num(0));
      }
...
//[[Rcpp::export]]
arma::drowvec runPMMH_arma(const double& sel_cof, const double& dom_par, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  log_lik(0) = calculateLogLikelihood_arma(sel_cof_chn(0), dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num);
