...
# call C++ functions
sourceCpp("./Code/Code v1.0/Code 1L/Code v1.0/CFUN_GEN.cpp")
...
simulateHMM <- function(model, sel_cof, dom_par, pop_siz, int_con, smp_gen, smp_siz, ...) {
  ...
  if (model == "WFD") {
    pop_mut_frq <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, int_con, int_gen, lst_gen, ptn_num, dat_aug = FALSE)
...
runPMMH <- function(sel_cof, dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num) {
  ...
  # run the PMMH
  sel_cof_chn <- runPMMH_arma(sel_cof, dom_par, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num)
