...
List runBPF_arma(const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    if (mut_frq_tmp(i) == 0 || mut_frq_tmp(i) == 1) {
      wght_tmp(i) = 0;
    } else {
...
// [[Rcpp::export]]
void calculateLogLikelihood_arma(double& log_lik, arma::drowvec& frq_pth, const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    if (mut_frq_pre(i) == 0 || mut_frq_pre(i) == 1) {
      wght(i) = 0;
    } else {
...
//[[Rcpp::export]]
List runPMMH_arma(const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::irowvec& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
