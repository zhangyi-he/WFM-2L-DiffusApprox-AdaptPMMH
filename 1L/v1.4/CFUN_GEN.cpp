...
//[[Rcpp::export]]
List runPMMH_arma(const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;

  arma::dmat sel_cof_chn = arma::zeros<arma::dmat>(2, itn_num);
  arma::dmat frq_pth_chn = arma::zeros<arma::dmat>(itn_num, arma::uword(max(smp_gen) - min(smp_gen)) * ptn_num + 1);
