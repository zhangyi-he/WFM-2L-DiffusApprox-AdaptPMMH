...
# call C++ functions
sourceCpp("./Code/Code v1.0/Code 1L/Code v1.4/CFUN_ALE.cpp")
...
simulateWFM <- function(sel_cof, dom_par, pop_siz, int_frq, evt_gen, int_gen, lst_gen) {
  if (evt_gen >= lst_gen) {
    ...
    fts_mat <- calculateFitnessMat_arma(sel_cof[1], dom_par)
    frq_pth_pre_evt <- simulateWFM_arma(fts_mat, pop_siz, int_frq, int_gen, evt_gen)
    frq_pth_pre_evt <- as.vector(frq_pth_pre_evt)

    fts_mat <- calculateFitnessMat_arma(sel_cof[2], dom_par)
    frq_pth_pst_evt <- simulateWFM_arma(fts_mat, pop_siz, tail(frq_pth_pre_evt, n = 1), evt_gen, lst_gen)
    frq_pth_pst_evt <- as.vector(frq_pth_pst_evt)
...
simulateWFD <- function(sel_cof, dom_par, pop_siz, ref_siz, int_frq, evt_gen, int_gen, lst_gen, ptn_num, dat_aug = TRUE) {
  if (evt_gen >= lst_gen) {
    frq_pth <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz, ref_siz, int_frq, int_gen, lst_gen, ptn_num)
    frq_pth <- as.vector(frq_pth)
  } else if (evt_gen < int_gen) {
    frq_pth <- simulateWFD_arma(sel_cof[2], dom_par, pop_siz, ref_siz, int_frq, int_gen, lst_gen, ptn_num)
    frq_pth <- as.vector(frq_pth)
  } else {
    frq_pth_pre_evt <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz, ref_siz, int_frq, int_gen, evt_gen, ptn_num)
    frq_pth_pre_evt <- as.vector(frq_pth_pre_evt)

    frq_pth_pst_evt <- simulateWFD_arma(sel_cof[2], dom_par, pop_siz, ref_siz, tail(frq_pth_pre_evt, n = 1), evt_gen, lst_gen, ptn_num)
    frq_pth_pst_evt <- as.vector(frq_pth_pst_evt)
...
simulateHMM <- function(model, sel_cof, dom_par, pop_siz, int_con, evt_gen, smp_gen, smp_siz, ...) {
  ...
  if (model == "WFM") {
    pop_frq <- cmpsimulateWFM(sel_cof, dom_par, pop_siz, int_con, evt_gen, int_gen, lst_gen)
  }
  if (model == "WFD") {
    pop_frq <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, ref_siz, int_con, evt_gen, int_gen, lst_gen, ptn_num, dat_aug = FALSE)
  }
