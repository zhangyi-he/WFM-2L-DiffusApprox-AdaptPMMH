...
arma::dmat simulateTLWFDS_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  ...
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    ...
    frq_pth.col(t) = frq_pth.col(t) / arma::accu(frq_pth.col(t));
  }
...
arma::dcolvec simulateWFD_arma(const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::dcolvec& int_frq, const double& dt, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  ...
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    ...
    arma::dcolvec mu_sel_b = -scl_sel_cof(0) * frq_pth_pre(2) * (frq_pth_pre(2) + 2 * frq_pth_pre(3)) * frq_pth_pre;
    arma::dcolvec mu_sel_c = scl_sel_cof(1) * (frq_pth_pre(1) + frq_pth_pre(3)) * frq_pth_pre;
    mu(0) = mu_sel_b(0) - mu_sel_c(0) * (frq_pth_pre(1) + frq_pth_pre(3)) - mu_rec;
    mu(1) = mu_sel_b(1) + mu_sel_c(1) * (frq_pth_pre(0) + frq_pth_pre(2)) + mu_rec;
    mu(2) = mu_sel_b(2) - mu_sel_c(2) * (frq_pth_pre(1) + frq_pth_pre(3)) + mu_rec + scl_sel_cof(0) * frq_pth_pre(2) * (frq_pth_pre(2) + frq_pth_pre(3));
    mu(3) = mu_sel_b(3) + mu_sel_c(3) * (frq_pth_pre(0) + frq_pth_pre(2)) - mu_rec + scl_sel_cof(0) * frq_pth_pre(2) * frq_pth_pre(3);
...
void calculateLogLikelihood_arma(double& log_lik, const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_smp_cnt, const double& dt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  arma::dcolvec scl_sel_cof_bd = scl_sel_cof.rows(0, 1);
  arma::dcolvec scl_sel_cof_ad = scl_sel_cof.rows(2, 3);
  ...
  for (arma::uword k = 1; k < dom_ind; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      part_pre.col(i) = simulateWFD_arma(scl_sel_cof_bd, scl_rec_rat, part_pst.col(i), dt, smp_gen(k - 1), smp_gen(k), ptn_num);
      pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_pre.col(i));
      for (arma::uword j = 0; j < smp_cnt.n_cols; j++) {
  ...
  if (smp_gen(dom_ind) != smp_gen(dom_ind - 1)) {
    for (arma::uword i = 0; i < pcl_num; i++) {
      part_pre.col(i) = simulateWFD_arma(scl_sel_cof_bd, scl_rec_rat, part_pst.col(i), dt, smp_gen(dom_ind - 1), smp_gen(dom_ind), ptn_num);
    }
    part_pst = part_pre;
  }
  ...
  for (arma::uword k = dom_ind + 1; k < smp_gen.n_elem; k++) {
    ...
    arma::imat smp_cnt = ptl_smp_cnt(k);
    for (arma::uword i = 0; i < pcl_num; i++) {
      part_pre.col(i) = simulateWFD_arma(scl_sel_cof_ad, scl_rec_rat, part_pst.col(i), dt, smp_gen(k - 1), smp_gen(k), ptn_num);
      pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_pre.col(i));
      for (arma::uword j = 0; j < smp_cnt.n_cols; j++) {
        wght(i) = wght(i) + calculateMultinomialProb_arma(smp_cnt.col(j), smp_siz(k), pop_frq);
      }
