...
arma::dmat simulateTLWFMS_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen) {
  ...
  for(arma::uword t = 1; t < arma::uword(lst_gen - int_gen) + 1; t++) {
    ...
    // viability selection
    gen_frq = fts % gen_frq;
    gen_frq = gen_frq / arma::accu(gen_frq);
    
    // meiosis (calculate the sampling probability)
    for(arma::uword i = 0; i < 4; i++) {
      hap_frq(i) = (sum(gen_frq.row(i)) + sum(gen_frq.col(i))) / 2;
    }
...
arma::dmat simulateTLWFDS_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  ...
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    ...
    mu(0) = -scl_sel_cof(0) * (frq_pth(2, t - 1) * frq_pth(0, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(2, t - 1)) - frq_pth(0, t - 1) * frq_pth(2, t - 1)) -
      scl_sel_cof(1) * (frq_pth(1, t - 1) * frq_pth(0, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(1, t - 1)) - frq_pth(0, t - 1) * frq_pth(1, t - 1)) -
      scl_sel_cof(2) * (frq_pth(0, t - 1) * (2 * frq_pth(1, t - 1) * frq_pth(2, t - 1) + frq_pth(3, t - 1) - frq_pth(3, t - 1) * frq_pth(3, t - 1))) -
      0.5 * scl_rec_rat * LD;
    mu(1) = -scl_sel_cof(0) * (frq_pth(2, t - 1) * frq_pth(1, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(2, t - 1))) -
      scl_sel_cof(1) * (frq_pth(1, t - 1) * frq_pth(1, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(1, t - 1)) - frq_pth(0, t - 1) * frq_pth(1, t - 1) - frq_pth(1, t - 1) * frq_pth(1, t - 1)) -
      scl_sel_cof(2) * (frq_pth(1, t - 1) * (2 * frq_pth(1, t - 1) * frq_pth(2, t - 1) + frq_pth(3, t - 1) - frq_pth(3, t - 1) * frq_pth(3, t - 1)) - frq_pth(1, t - 1) * frq_pth(2, t - 1)) +
      0.5 * scl_rec_rat * LD;
    mu(2) = -scl_sel_cof(0) * (frq_pth(2, t - 1) * frq_pth(2, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(2, t - 1)) - frq_pth(0, t - 1) * frq_pth(2, t - 1) - frq_pth(2, t - 1) * frq_pth(2, t - 1)) -
      scl_sel_cof(1) * (frq_pth(1, t - 1) * frq_pth(2, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(1, t - 1))) -
      scl_sel_cof(2) * (frq_pth(2, t - 1) * (2 * frq_pth(1, t - 1) * frq_pth(2, t - 1) + frq_pth(3, t - 1) - frq_pth(3, t - 1) * frq_pth(3, t - 1)) - frq_pth(1, t - 1) * frq_pth(2, t - 1)) +
      0.5 * scl_rec_rat * LD;
    mu(3) = -scl_sel_cof(0) * (frq_pth(2, t - 1) * frq_pth(3, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(2, t - 1))) -
      scl_sel_cof(1) * (frq_pth(1, t - 1) * frq_pth(3, t - 1) * (2 * frq_pth(0, t - 1) + frq_pth(1, t - 1))) -
      scl_sel_cof(2) * (frq_pth(3, t - 1) * (2 * frq_pth(1, t - 1) * frq_pth(2, t - 1) + frq_pth(3, t - 1) - frq_pth(3, t - 1) * frq_pth(3, t - 1)) - frq_pth(3, t - 1) + frq_pth(3, t - 1) * frq_pth(3, t - 1)) -
      0.5 * scl_rec_rat * LD;
...
/********** CVT **********/
// Convert the haplotype frequency trajectories to the phased genotype frequency trajectories
// [[Rcpp::export]]
arma::dmat convertHaploPath2PhasedGenoPath_arma(const arma::dmat& hap_frq_pth) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  // declare the phased genotype frequency trajectories
  arma::dmat frq_pth(10, hap_frq_pth.n_cols);
  
  for (arma::uword t = 0; t < hap_frq_pth.n_cols; t++) {
    arma::dcolvec hap_frq = hap_frq_pth.col(t);
    arma::dmat gen_frq = hap_frq * hap_frq.t();
...
// Convert the haplotype frequency trajectories to the unphased genotype frequency trajectories
// [[Rcpp::export]]
arma::dmat convertHaploPath2UnphasedGenoPath_arma(const arma::dmat& hap_frq_pth) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  // declare the unphased genotype frequency trajectories
  arma::dmat frq_pth(9, hap_frq_pth.n_cols);
  ...
  for (arma::uword t = 0; t < hap_frq_pth.n_cols; t++) {
    ...
    frq_pth(2, t) = gen_frq(1, 1);
    frq_pth(3, t) = gen_frq(0, 2) + gen_frq(2, 0);
    frq_pth(4, t) = gen_frq(0, 3) + gen_frq(1, 2) + gen_frq(2, 1) + gen_frq(3, 0);
    frq_pth(5, t) = gen_frq(1, 3) + gen_frq(3, 1);
    frq_pth(6, t) = gen_frq(2, 2);
    frq_pth(7, t) = gen_frq(2, 3) + gen_frq(3, 2);
    frq_pth(8, t) = gen_frq(3, 3);
...
// Calculate the possible unobserved sample phased genotype counts from the observed sample unphased genotype/phenotype counts with uncertainties
// [[Rcpp::export]]
...
/********** EMN **********/
// Convert the haplotype frequencies to the phased genotype frequencies
// [[Rcpp::export]]
arma::dcolvec convertHaploFreq2PhasedGenoFreq_arma(const arma::dcolvec& hap_frq) {
  // ensure RNG gets set/reset
  RNGScope scope;
  
  // declare the phased genotype frequencies
  arma::dcolvec frq(10);
  
  arma::dmat gen_frq = hap_frq * hap_frq.t();
  frq(0) = gen_frq(0, 0);
  frq(1) = gen_frq(0, 1) + gen_frq(1, 0);
  frq(2) = gen_frq(1, 1);
  frq(3) = gen_frq(0, 2) + gen_frq(2, 0);
  frq(4) = gen_frq(1, 2) + gen_frq(2, 1);
  frq(5) = gen_frq(1, 3) + gen_frq(3, 1);
  frq(6) = gen_frq(2, 2);
  frq(7) = gen_frq(2, 3) + gen_frq(3, 2);
  frq(8) = gen_frq(3, 3);
  frq(9) = gen_frq(0, 3) + gen_frq(3, 0);
...
List runBPF_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::imat& mis_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  for (arma::uword i = 0; i < pcl_num; i++) {
    arma::dcolvec pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_tmp.col(i));
    for (arma::uword j = 0; j < ptl_smp_cnt.n_cols; j++) {
      wght_tmp(i) = wght_tmp(i) + calculateMultinomialProb_arma(ptl_smp_cnt.col(j), smp_siz(0), pop_frq) / smp_cnt.n_cols;
    }
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateTLWFDS_arma(sel_cof, rec_rat, pop_siz, part_tmp.col(i), smp_gen(k - 1), smp_gen(k), ptn_num);
      part_tmp.col(i) = arma::vectorise(path.tail_cols(1), 0);
      arma::dcolvec pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_tmp.col(i));
      for (arma::uword j = 0; j < ptl_smp_cnt.n_cols; j++) {
        wght_tmp(i) = wght_tmp(i) + calculateMultinomialProb_arma(ptl_smp_cnt.col(j), smp_siz(k), pop_frq) / smp_cnt.n_cols;
      }
    }
...
/********** PMMH **********/
// Simulate the haplotype frequency trajectories in a pre-determined generation according to the two-locus Wright-Fisher diffusion with selection using the Euler-Maruyama method
// [[Rcpp::export]]
arma::dcolvec simulateWFD_arma(const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::dcolvec& int_frq, const double& dt, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  // declare the haplotype frequency trajectories
  arma::dcolvec frq_pth_pre = int_frq;
  arma::dcolvec frq_pth_pst = int_frq;
  
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    // calculate the drift coefficient vector
    arma::dcolvec mu(4);
    double mu_rec =  0.5 * scl_rec_rat * (frq_pth_pre(0) * frq_pth_pre(3) - frq_pth_pre(1) * frq_pth_pre(2));
    arma::dcolvec mu_sel_to = -scl_sel_cof(0) * frq_pth_pre(2) * (2 * frq_pth_pre(0) + frq_pth_pre(2))* frq_pth_pre;
    arma::dcolvec mu_sel_sb = -scl_sel_cof(1) * frq_pth_pre(1) * (2 * frq_pth_pre(0) + frq_pth_pre(1))* frq_pth_pre;
    arma::dcolvec mu_sel_ts = -scl_sel_cof(2) * (2 * frq_pth_pre(1) * frq_pth_pre(2) + frq_pth_pre(3) - frq_pth_pre(3) * frq_pth_pre(3)) * frq_pth_pre;
    mu(0) = mu_sel_to(0) + mu_sel_sb(0) + mu_sel_ts(0) - mu_rec + scl_sel_cof(0) * frq_pth_pre(0) * frq_pth_pre(2) + scl_sel_cof(1) * frq_pth_pre(1) * frq_pth_pre(0);
    mu(1) = mu_sel_to(1) + mu_sel_sb(1) + mu_sel_ts(1) + mu_rec + scl_sel_cof(1) * frq_pth_pre(1) * (frq_pth_pre(0) + frq_pth_pre(1)) + scl_sel_cof(2) * frq_pth_pre(1) * frq_pth_pre(2);
    mu(2) = mu_sel_to(2) + mu_sel_sb(2) + mu_sel_ts(2) + mu_rec + scl_sel_cof(0) * frq_pth_pre(2) * (frq_pth_pre(0) + frq_pth_pre(2)) + scl_sel_cof(2) * frq_pth_pre(1) * frq_pth_pre(2);
    mu(3) = mu_sel_to(3) + mu_sel_sb(3) + mu_sel_ts(3) - mu_rec + scl_sel_cof(2) * (frq_pth_pre(3) - frq_pth_pre(3) * frq_pth_pre(3));
    
    // calculate the diffusion coefficient matrix
    arma::dmat sigma = arma::zeros<arma::dmat>(4, 6);
    sigma(0, 0) = pow(frq_pth_pre(0) * frq_pth_pre(1), 0.5);
    sigma(0, 1) = pow(frq_pth_pre(0) * frq_pth_pre(2), 0.5);
    sigma(0, 2) = pow(frq_pth_pre(0) * frq_pth_pre(3), 0.5);
    //sigma(0, 3) = 0;
    //sigma(0, 4) = 0;
    //sigma(0, 5) = 0;
    sigma(1, 0) = -pow(frq_pth_pre(1) * frq_pth_pre(0), 0.5);
    //sigma(1, 1) = 0;
    //sigma(1, 2) = 0;
    sigma(1, 3) = pow(frq_pth_pre(1) * frq_pth_pre(2), 0.5);
    sigma(1, 4) = pow(frq_pth_pre(1) * frq_pth_pre(3), 0.5);
    //sigma(1, 5) = 0;
    //sigma(2, 0) = 0;
    sigma(2, 1) = -pow(frq_pth_pre(2) * frq_pth_pre(0), 0.5);
    //sigma(2, 2) = 0;
    sigma(2, 3) = -pow(frq_pth_pre(2) * frq_pth_pre(1), 0.5);
    //sigma(2, 4) = 0;
    sigma(2, 5) = pow(frq_pth_pre(2) * frq_pth_pre(3), 0.5);
    //sigma(3, 0) = 0;
    //sigma(3, 1) = 0;
    sigma(3, 2) = -pow(frq_pth_pre(3) * frq_pth_pre(0), 0.5);
    //sigma(3, 3) = 0;
    sigma(3, 4) = -pow(frq_pth_pre(3) * frq_pth_pre(1), 0.5);
    sigma(3, 5) = -pow(frq_pth_pre(3) * frq_pth_pre(2), 0.5);
    
    // proceed the Euler-Maruyama scheme
    frq_pth_pst = frq_pth_pre + mu * dt + sigma * dW.col(t - 1);
    ...
    frq_pth_pst = frq_pth_pst / arma::accu(frq_pth_pst);
    frq_pth_pre = frq_pth_pst;
  }
...
// [[Rcpp::export]]
void calculateLogLikelihood_arma(double& log_lik, const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_smp_cnt, const double& dt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  arma::dcolvec pop_frq = arma::zeros<arma::dcolvec>(4);
  
  // initialise the particles
  part_pre = arma::normalise(arma::randu<arma::dmat>(4, pcl_num), 1, 0);
  arma::imat smp_cnt = ptl_smp_cnt(0);
  for (arma::uword i = 0; i < pcl_num; i++) {
    pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_pre.col(i));
    for (arma::uword j = 0; j < smp_cnt.n_cols; j++) {
      wght(i) = wght(i) + calculateMultinomialProb_arma(smp_cnt.col(j), smp_siz(0), pop_frq);
    }
  }
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      part_pre.col(i) = simulateWFD_arma(scl_sel_cof, scl_rec_rat, part_pst.col(i), dt, smp_gen(k - 1), smp_gen(k), ptn_num);
      pop_frq = convertHaploFreq2PhasedGenoFreq_arma(part_pre.col(i));
      for (arma::uword j = 0; j < smp_cnt.n_cols; j++) {
        wght(i) = wght(i) + calculateMultinomialProb_arma(smp_cnt.col(j), smp_siz(k), pop_frq);
      }
    }
...
List calculateOptimalParticleNum_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::imat& mis_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& gap_num) {
  ...
  for (arma::uword i = 0; i < 300; i++) {
    calculateLogLikelihood_arma(log_lik(i), scl_sel_cof, scl_rec_rat, smp_gen, smp_siz, ptl_smp_cnt, dt, ptn_num, pcl_num);
  }
  ...
  if (log_lik_sdv(0) > 1.7) {
    while (log_lik_sdv(0) > 1.0 && opt_pcl_num(0) > gap_num) {
      ...
      for (arma::uword i = 0; i < 300; i++) {
        calculateLogLikelihood_arma(log_lik(i), scl_sel_cof, scl_rec_rat, smp_gen, smp_siz, ptl_smp_cnt, dt, ptn_num, opt_pcl_num(0));
      }
...
// [[Rcpp::export]]
void runMH_arma(bool& apt, double& log_lik, const double& log_lik_pre, const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_smp_cnt, const double& dt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
arma::dmat runPMMH_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::imat& mis_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::dmat sel_cof_chn = arma::zeros<arma::dmat>(3, itn_num);
  ...
  // rescale the selection coefficients and the recombination rate
  arma::dcolvec scl_sel_cof = 2 * pop_siz * sel_cof_chn.col(0);
  double scl_rec_rat = 4 * pop_siz * rec_rat;
  ...
  arma::dmat sel_cof_cov = arma::eye<arma::dmat>(3, 3) * pow(sel_cof_sd, 2);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    if (arma::any(sel_cof_chn.col(i) < -1)) {
      ...
      arma::dcolvec scl_sel_cof = 2 * pop_siz * sel_cof_chn.col(i);
      runMH_arma(apt, log_lik_chn(i), log_lik_chn(i - 1), scl_sel_cof, scl_rec_rat, smp_gen, smp_siz, ptl_smp_cnt, dt, ptn_num, pcl_num);
      if (apt == false) {
        sel_cof_chn.col(i) = sel_cof_chn.col(i - 1);
      }
    }
...
arma::dmat runPAPMMH_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::imat& mis_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num, const arma::uword nap_num) {
  ...
  for (arma::uword i = 1; i < nap_num; i++) {
    ...
    if (arma::any(sel_cof_chn.col(i) < -1)) {
      ...
      //log_apt_rto = arma::as_scalar(log(arma::randu())) - log_pri_chn(i) + log_pri_chn(i - 1);
      //log_apt_rto = arma::as_scalar(log(arma::randu())) - log_pri_chn(i) - log_psl_old_new + log_pri_chn(i - 1) + log_psl_new_old;
      scl_sel_cof = 2 * pop_siz * sel_cof_chn.col(i);
      runMH_arma(apt, log_lik_chn(i), log_lik_chn(i - 1), scl_sel_cof, scl_rec_rat, smp_gen, smp_siz, ptl_smp_cnt, dt, ptn_num, pcl_num);
      if (apt == false) {
        sel_cof_chn.col(i) = sel_cof_chn.col(i - 1);
      }
  ...
  for (arma::uword i = nap_num; i < itn_num; i++) {
    ...
    // draw the candidates of the selection coefficients from the proposals
    sel_cof_cov = (1 - 1.0 / (i - 1)) * sel_cof_cov + (2.88 / (i - 1)) * ((i - 2) * sel_cof_mat_pre - (i - 1) * sel_cof_mat_pst + sel_cof_chn.col(i - 1) * arma::trans(sel_cof_chn.col(i - 1)) + 1e-06 * arma::eye<arma::dmat>(3, 3));
    sel_cof_chn.col(i) = arma::mvnrnd(sel_cof_chn.col(i - 1), sel_cof_cov);
    if (arma::any(sel_cof_chn.col(i) < -1)) {
      sel_cof_chn.col(i) = sel_cof_chn.col(i - 1);
      log_lik_chn(i) = log_lik_chn(i - 1);
...
// [[Rcpp::export]]
void runERMH_arma(bool& apt, double& log_lik, const double& log_lik_pre, const arma::dcolvec& scl_sel_cof, const double& scl_rec_rat, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_smp_cnt, const double& dt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
arma::dmat runERPMMH_arma(const arma::dcolvec& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::imat& mis_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    if (arma::any(sel_cof_chn.col(i) < -1)) {
      ...
      //log_apt_rto = arma::as_scalar(log(arma::randu())) - log_pri_chn(i) - log_psl_old_new + log_pri_chn(i - 1) + log_psl_new_old;
      arma::dcolvec scl_sel_cof = 2 * pop_siz * sel_cof_chn.col(i);
      runERMH_arma(apt, log_lik_chn(i), log_lik_chn(i - 1), scl_sel_cof, scl_rec_rat, smp_gen, smp_siz, ptl_smp_cnt, dt, ptn_num, pcl_num);
      if (apt == false) {
        sel_cof_chn.col(i) = sel_cof_chn.col(i - 1);
      }
    }
