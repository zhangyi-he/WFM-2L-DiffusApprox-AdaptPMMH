...
#' version 1.0
#' Phenotypes controlled by two genes with epistatic interaction
...
simulateHMM <- function(model, sel_cof, rec_rat, pop_siz, int_con, evt_gen, smp_lab, smp_gen, smp_qua, thr_val, ...) {
  ...
  colnames(tru_smp) <- c("generation", "AA", "Aa", "aa", "EE", "Ee", "ee")
  tru_smp <- tru_smp[order(tru_smp$generation), ]

  raw_smp <- tru_smp
  sel_idx <- which(tru_smp$'AA' == 1)
  ...
  sel_idx <- which(raw_smp$'AA' / raw_smp$'Aa' >= thr_val & raw_smp$'AA' / raw_smp$'aa' >= thr_val)
  if (length(sel_idx) > 1) {
  ...
  mis_idx <- 1:nrow(raw_smp)
  sel_idx <- which(raw_smp$'EE' / raw_smp$'Ee' >= thr_val & raw_smp$'EE' / raw_smp$'ee' >= thr_val)
  if (length(sel_idx) > 1) {
...
runBPF <- function(sel_cof, rec_rat, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, pcl_num) {
  ...
  # colnames(raw_smp) <- c("generation", "AA", "Aa", "aa", "EE", "Ee", "ee")
  raw_smp <- as.data.frame(cbind(raw_smp[, 1],
    ...
    raw_smp[, 4] * raw_smp[, 6],
    raw_smp[, 4] * raw_smp[, 7]))
  colnames(raw_smp) <- c("generation", "AA/EE", "AA/Ee", "AA/ee", "Aa/EE", "Aa/Ee", "aa/EE", "Aa/ee", "aa/Ee", "aa/ee")
