...
# call C++ functions
sourceCpp("./Code/Code v1.0/GenoEvo/Horse coat colour/HE2017_cfun.cpp")
...
#' Standard version
simulateTLWFMS <- function(type, phased, sel_cof, rec_rat, pop_siz, int_frq, dom_gen, int_gen, lst_gen) {
  if (type == "allele") {
    if (dom_gen >= lst_gen) {
...
#' Standard version
simulateTLWFDS <- function(type, phased, sel_cof, rec_rat, pop_siz, int_frq, dom_gen, int_gen, lst_gen, ptn_num) {
  if (type == "allele") {
    if (dom_gen >= lst_gen) {
...
simulateHMM <- function(model, missing, sel_cof, rec_rat, pop_siz, int_frq, dom_gen, smp_gen, smp_siz, ...) {
  ...
  if (model == "WFM") {
    pop_frq <- cmpsimulateTLWFMS(type = "genotype", phased = TRUE, sel_cof, rec_rat, pop_siz, int_frq, dom_gen, int_gen, lst_gen)
  }
  if (model == "WFD") {
    pop_frq <- cmpsimulateTLWFDS(type = "genotype", phased = TRUE, sel_cof, rec_rat, pop_siz, int_frq, dom_gen, int_gen, lst_gen, ptn_num)
  }
