...
reg_var <- matrix(NA, nrow = 4, ncol = max(smp_gen) - min(smp_gen) + 1)
reg_var[1, ] <- rep(1, length.out = max(smp_gen) - min(smp_gen) + 1)
reg_var[2, ] <- (min(smp_gen) + 0:((max(smp_gen) - min(smp_gen)))) / (max(smp_gen) - min(smp_gen))
...
fit <- lm(sel_cof[1, ] ~ 1 + reg_var[2, ] + reg_var[3, ] + reg_var[4, ])
reg_cof[1, ] <- fit$coefficients
fit <- lm(sel_cof[2, ] ~ 1 + reg_var[2, ] + reg_var[3, ] + reg_var[4, ])
reg_cof[2, ] <- fit$coefficients
sel_cof <- reg_cof %*% reg_var
rec_rat <- 5e-01
pop_siz <- 5e+03
int_con <- c(5e-01, 2e-01, 2e-01, 1e-01)
smp_siz <- rep(100, 11)
...
save(sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, BPF,
     file = "./Output/Output v1.0/Test v1.0/TEST_COL_BPF.rda")

load("./Output/Output v1.0/Test v1.0/TEST_COL_BPF.rda")
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_BPF_Likelihood.pdf", width = 8, height = 6)
...
save(sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, gap_num, OptNum,
     file = "./Output/Output v1.0/Test v1.0/TEST_COL_OptNum.rda")

load("./Output/Output v1.0/Test v1.0/TEST_COL_OptNum.rda")
...
save(reg_cof, sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num, PMMH,
     file = "./Output/Output v1.0/Test v1.0/TEST_COL_PMMH.rda")

load("./Output/Output v1.0/Test v1.0/TEST_COL_PMMH.rda")

reg_cof_chn <- PMMH$reg_cof_chn
sel_cof_chn <- PMMH$sel_cof_chn

pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_PMMH_Traceplot.pdf", width = 16, height = 24)
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_PMMH_Autocorrplot.pdf", width = 32, height = 24)
...
autocorr.plot(reg_cof_chn[1, 1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 0th-order Taylor series coef of black before thinning")
autocorr.plot(reg_cof_chn[1, 2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 1st-order Taylor series coef of black before thinning")
autocorr.plot(reg_cof_chn[1, 3, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 2nd-order Taylor series coef of black before thinning")
autocorr.plot(reg_cof_chn[1, 4, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 3rd-order Taylor series coef of black before thinning")
autocorr.plot(reg_cof_chn[2, 1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 0th-order Taylor series coef of chestnut before thinning")
autocorr.plot(reg_cof_chn[2, 2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 1st-order Taylor series coef of chestnut before thinning")
autocorr.plot(reg_cof_chn[2, 3, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 2nd-order Taylor series coef of chestnut before thinning")
autocorr.plot(reg_cof_chn[2, 4, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 3rd-order Taylor series coef of chestnut before thinning")

thn_num <- 8e+00
reg_cof_chn <- reg_cof_chn[, , (1:round(dim(reg_cof_chn)[3] / thn_num)) * thn_num]

autocorr.plot(reg_cof_chn[1, 1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 0th-order Taylor series coef of black after thinning")
autocorr.plot(reg_cof_chn[1, 2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 1st-order Taylor series coef of black after thinning")
autocorr.plot(reg_cof_chn[1, 3, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 2nd-order Taylor series coef of black after thinning")
autocorr.plot(reg_cof_chn[1, 4, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 3rd-order Taylor series coef of black after thinning")
autocorr.plot(reg_cof_chn[2, 1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 0th-order Taylor series coef of chestnut after thinning")
autocorr.plot(reg_cof_chn[2, 2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 1st-order Taylor series coef of chestnut after thinning")
autocorr.plot(reg_cof_chn[2, 3, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 2nd-order Taylor series coef of chestnut after thinning")
autocorr.plot(reg_cof_chn[2, 4, ], lag.max = 50, auto.layout = FALSE, main = "Autocorr plot for 3rd-order Taylor series coef of chestnut after thinning")
dev.off()
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_PMMH_Posterior.pdf", width = 16, height = 24)
par(mfcol = c(4, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(reg_cof_chn[1, 1, ], breaks = seq(min(reg_cof_chn[1, 1, ]), max(reg_cof_chn[1, 1, ]), length.out = 50), freq = FALSE,
     xlab = "0th-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of black")
...
hist(reg_cof_chn[1, 2, ], breaks = seq(min(reg_cof_chn[1, 2, ]), max(reg_cof_chn[1, 2, ]), length.out = 50), freq = FALSE,
     xlab = "1st-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of black")
...
hist(reg_cof_chn[1, 3, ], breaks = seq(min(reg_cof_chn[1, 3, ]), max(reg_cof_chn[1, 3, ]), length.out = 50), freq = FALSE,
     xlab = "2nd-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of black")
...
hist(reg_cof_chn[1, 4, ], breaks = seq(min(reg_cof_chn[1, 4, ]), max(reg_cof_chn[1, 4, ]), length.out = 50), freq = FALSE,
     xlab = "3rd-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of black")
lines(density(reg_cof_chn[1, 1, ]), lwd = 2, col = 'black')
...
hist(reg_cof_chn[2, 1, ], breaks = seq(min(reg_cof_chn[2, 1, ]), max(reg_cof_chn[2, 1, ]), length.out = 50), freq = FALSE,
     xlab = "0th-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of chestnut")
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_PMMH_TaylorSeries.pdf", width = 8, height = 12)
...
save(reg_cof, sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num, stp_siz, apt_rto, AdaptPMMH,
     file = "./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH.rda")

load("./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH.rda")

reg_cof_chn <- AdaptPMMH$reg_cof_chn
sel_cof_chn <- AdaptPMMH$sel_cof_chn

pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH_Traceplot.pdf", width = 16, height = 24)
par(mfcol = c(4, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(1:itn_num, reg_cof_chn[1, 1, 1:itn_num], type = 'l',
     xlab = "Iteration", ylab = "0th-order Taylor series coefficient",
     main = "Trace plot for Taylor series coef of black")
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH_Autocorrplot.pdf", width = 32, height = 24)
par(mfcol = c(4, 4), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
brn_num <- 1e+04
reg_cof_chn <- reg_cof_chn[, , brn_num:dim(reg_cof_chn)[3]]
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH_Posterior.pdf", width = 16, height = 24)
par(mfcol = c(4, 2), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
hist(reg_cof_chn[1, 1, ], breaks = seq(min(reg_cof_chn[1, 1, ]), max(reg_cof_chn[1, 1, ]), length.out = 50), freq = FALSE,
     xlab = "0th-order Taylor series coefficient",
     main = "Marginal posterior for Taylor series coef of black")
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_AdaptPMMH_TaylorSeries.pdf", width = 8, height = 12)
par(mfcol = c(2, 1), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(0, type = "n", xlim = c(min(smp_gen), max(smp_gen)), ylim = c(min(sel_cof, sel_cof_chn[1, , ]), max(sel_cof, sel_cof_chn[1, , ])),
     main = "Temporal changes in the selection coef of black", xlab = "Generation", ylab = "Selection coefficient")
for (i in 1:dim(sel_cof_chn)[3]) {
...
pdf(file = "./Output/Output v1.0/Test v1.0/TEST_COL_BayesProc_TaylorSeries.pdf", width = 8, height = 12)
par(mfcol = c(2, 1), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
plot(0, type = "n", xlim = c(min(smp_gen), max(smp_gen)), ylim = c(min(sel_cof, sel_cof_chn[1, , ]), max(sel_cof, sel_cof_chn[1, , ])),
     main = "Temporal changes in the selection coef of black", xlab = "Generation", ylab = "Selection coefficient")
for (i in 1:dim(sel_cof_chn)[3]) {
