// A Bayesian approach for estimating time-varying selection coefficients from ancient DNA data
...
// [[Rcpp::export]]
List simulateWFM_arma(const arma::dmat& sel_cof, const double& rec_rat, const int& pop_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen) {
  ...
  for (arma::uword k = 1; k < arma::uword(lst_gen - int_gen) + 1; k++) {
    ...
    fts_mat = calculateFitnessMat_arma(sel_cof.col(k - 1));
    ...
    fts_mat = calculateFitnessMat_arma(sel_cof.col(k));
...
// [[Rcpp::export]]
arma::dmat simulateWFD_arma(const arma::dmat& sel_cof, const double& rec_rat, const int& pop_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;

  // rescale the selection coefficients and the recombination rate
  arma::dmat scl_sel_cof = 2 * pop_siz * sel_cof;
  ...
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    ...
    mu(0) = -scl_sel_cof(0, t - 1) * frq_pth(2, t - 1) * (frq_pth(0, t - 1) * frq_pth(3, t - 1) + frq_pth(0, t - 1) * (frq_pth(2, t - 1) + frq_pth(3, t - 1))) -
      scl_sel_cof(1, t - 1) * frq_pth(0, t - 1) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) -
      0.5 * scl_rec_rat * LD;
    mu(1) = -scl_sel_cof(0, t - 1) * frq_pth(2, t - 1) * (frq_pth(1, t - 1) * frq_pth(3, t - 1) + frq_pth(1, t - 1) * (frq_pth(2, t - 1) + frq_pth(3, t - 1))) +
      scl_sel_cof(1, t - 1) * frq_pth(1, t - 1) * (frq_pth(0, t - 1) + frq_pth(2, t - 1)) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) +
      0.5 * scl_rec_rat * LD;
    mu(2) = -scl_sel_cof(0, t - 1) * frq_pth(2, t - 1) * (frq_pth(2, t - 1) * frq_pth(3, t - 1) + frq_pth(2, t - 1) * (frq_pth(2, t - 1) + frq_pth(3, t - 1)) - (frq_pth(2, t - 1) + frq_pth(3, t - 1))) -
      scl_sel_cof(1, t - 1) * frq_pth(2, t - 1) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) +
      0.5 * scl_rec_rat * LD;
    mu(3) = -scl_sel_cof(0, t - 1) * frq_pth(2, t - 1) * (frq_pth(3, t - 1) * frq_pth(3, t - 1) + frq_pth(3, t - 1) * (frq_pth(2, t - 1) + frq_pth(3, t - 1)) - frq_pth(3, t - 1)) +
      scl_sel_cof(1, t - 1) * frq_pth(3, t - 1) * (frq_pth(0, t - 1) + frq_pth(2, t - 1)) * (frq_pth(1, t - 1) + frq_pth(3, t - 1)) -
      0.5 * scl_rec_rat * LD;
...
// [[Rcpp::export]]
List runBPF_arma(const arma::dmat& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  arma::dmat fts_mat = calculateFitnessMat_arma(sel_cof.col(0));
  // arma::dmat fts_mat = calculateFitnessMat_arma(sel_cof.col((smp_gen(0) - smp_gen(0)) * ptn_num)));
  for (arma::uword i = 0; i < pcl_num; i++) {
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    arma::dmat fts_mat = calculateFitnessMat_arma(sel_cof.col((smp_gen(k) - smp_gen(0)) * ptn_num));
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateWFD_arma(sel_cof.cols((smp_gen(k-1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num), rec_rat, pop_siz, hap_frq_tmp.col(i), smp_gen(k - 1), smp_gen(k), ptn_num);
...
// [[Rcpp::export]]
double calculateLogLikelihood_arma(const arma::dmat& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  for (arma::uword k = 1; k < smp_gen.n_elem; k++) {
    ...
    arma::dmat fts_mat = calculateFitnessMat_arma(sel_cof.col((smp_gen(k) - smp_gen(0)) * ptn_num));
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateWFD_arma(sel_cof.cols((smp_gen(k-1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num), rec_rat, pop_siz, hap_frq_pst.col(i), smp_gen(k - 1), smp_gen(k), ptn_num);
...
// [[Rcpp::export]]
List calculateOptimalParticleNum_arma(const arma::dmat& sel_cof, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& gap_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
//[[Rcpp::export]]
arma::dcube runPMMH_arma(const arma::dmat& reg_cof, const arma::dmat& reg_var, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  arma::dcube reg_cof_chn = arma::zeros<arma::dcube>(2, 4, itn_num);
  ...
  arma::dmat reg_cof_sd = 1e-02 * arma::ones<arma::dmat>(2, 4);
  ...
  reg_cof_chn.slice(0) = reg_cof;

  arma::dmat sig_cof = reg_cof_chn.slice(0);
  arma::dmat sel_cof = reg_var;
  for (arma::uword j = 0; j < sel_cof.n_rows; j++) {
    sel_cof.row(j) = sig_cof(j, 0) + sig_cof(j, 1) / (1 + exp(-sig_cof(j, 2) * (reg_var.row(j) - sig_cof(j, 3))));
  }
  log_lik(0) = calculateLogLikelihood_arma(sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    reg_cof_chn.slice(i) = reg_cof_chn.slice(i - 1) + reg_cof_sd % arma::randn<arma::dmat>(2, 4);

    // calculate the likelihood
    sig_cof = reg_cof_chn.slice(i);
    for (arma::uword j = 0; j < sel_cof.n_rows; j++) {
      sel_cof.row(j) = sig_cof(j, 0) + sig_cof(j, 1) / (1 + exp(-sig_cof(j, 2) * (reg_var.row(j) - sig_cof(j, 3))));
    }
    if (arma::any(arma::vectorise(sel_cof) < -1)) {
      reg_cof_chn.slice(i) = reg_cof_chn.slice(i - 1);
      log_lik(1) = log_lik(0);
      // apt_cnt = apt_cnt + 0;
      cout << "acceptance: " << apt_cnt / i << endl;
    } else {
      log_lik(1) = calculateLogLikelihood_arma(sel_cof, rec_rat, pop_siz, smp_gen, smp_siz, ptl_cnt, ptn_num, pcl_num);
...
arma::dcube runAdaptPMMH_arma(const arma::dmat& reg_cof, const arma::dmat& reg_var, const double& rec_rat, const int& pop_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::imat& smp_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num, const arma::drowvec& stp_siz, double& apt_rto) {
  ...
  arma::dcube reg_cof_chn = arma::zeros<arma::dcube>(2, 4, itn_num);
  arma::dcolvec reg_cof_tmp = arma::vectorise(reg_cof);

  // arma::drowvec log_pri = arma::zeros<arma::drowvec>(2);
  arma::drowvec log_lik = arma::zeros<arma::drowvec>(2);
  //arma::drowvec log_psl = arma::zeros<arma::drowvec>(2);

  arma::dcolvec U = arma::zeros<arma::dcolvec>(8);
  arma::dmat S = 1e-02 * arma::eye<arma::dmat>(8, 8);
  arma::dmat M = arma::zeros<arma::dmat>(8, 8);
  arma::dmat I = arma::eye<arma::dmat>(8, 8);
  ...
  // or take the beta prior with alpha = 1 and beta = 3
  reg_cof_chn.slice(0) = reg_cof;
  // reg_cof_chn.slice(0) = arma::reshape(reg_cof_tmp, 2, 4);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    // draw the candidates of the selection coefficients from the random walk proposal
    U = arma::randn<arma::dcolvec>(8);
    reg_cof_tmp = reg_cof_tmp + S * U;
    reg_cof_chn.slice(i) = arma::reshape(reg_cof_tmp, 2, 4);
    ...
    reg_cof_tmp = arma::vectorise(reg_cof_chn.slice(i));
