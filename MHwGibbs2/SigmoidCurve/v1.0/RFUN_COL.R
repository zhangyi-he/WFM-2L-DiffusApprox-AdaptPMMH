...
# call C++ functions
sourceCpp("./Code/Code v1.0/Code v2.0/CFUN_COL.cpp")
...
runPMMH <- function(reg_cof, rec_rat, pop_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num) {
  reg_var <- matrix(NA, nrow = 2, ncol = (max(smp_gen) - min(smp_gen)) * ptn_num + 1)
  reg_var[1, ] <- (min(smp_gen) + 0:((max(smp_gen) - min(smp_gen)) * ptn_num) / ptn_num) / (max(smp_gen) - min(smp_gen))
  reg_var[2, ] <- reg_var[1, ]
  ...
  # reg_cof_chn[, 1, ]
  # reg_cof_chn[, 2, ]
  reg_cof_chn[, 3, ] <- reg_cof_chn[, 3, ] / (max(smp_gen) - min(smp_gen))
  reg_cof_chn[, 4, ] <- reg_cof_chn[, 4, ] * (max(smp_gen) - min(smp_gen))

  reg_var <- min(smp_gen):max(smp_gen)

  sel_cof_chn <- array(NA, dim = c(2, max(smp_gen) - min(smp_gen) + 1, itn_num))
  for (i in 1:itn_num) {
    sel_cof_chn[1, , i] <- reg_cof_chn[1, 1, i] + reg_cof_chn[1, 2, i] / (1 + exp(-reg_cof_chn[1, 3, i] * (reg_var - reg_cof_chn[1, 4, i])))
    sel_cof_chn[2, , i] <- reg_cof_chn[2, 1, i] + reg_cof_chn[2, 2, i] / (1 + exp(-reg_cof_chn[2, 3, i] * (reg_var - reg_cof_chn[2, 4, i])))
  }
