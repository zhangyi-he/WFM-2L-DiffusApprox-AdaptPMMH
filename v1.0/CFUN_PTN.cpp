// Estimating selection coefficients and testing their changes from ancient DNA data with the flexibility of modelling linkage and epistasis
...
// [[Rcpp::export]]
arma::imat imputeSample(const arma::dmat& raw_smp, const arma::dmat& sel_cof, const int& evt_gen, const arma::dmat& hap_pth) {
  // ensure RNG gets set/reset
  RNGScope scope;

  arma::irowvec smp_gen = arma::conv_to<arma::irowvec>::from(raw_smp.row(0));
  arma::dmat gen_lik = raw_smp.rows(1, 9);

  arma::urowvec smp_idx = arma::conv_to<arma::urowvec>::from(smp_gen - smp_gen.min());
  arma::imat smp_cnt = arma::zeros<arma::imat>(9, raw_smp.n_cols);

  arma::dmat fts_mat = arma::ones<arma::dmat>(4, 4);
  if (arma::all(smp_gen > evt_gen)) {
    fts_mat = calculateFitnessMat_arma(sel_cof.col(1));
    for (arma::uword i = 0; i < smp_gen.n_elem; i++) {
      arma::dcolvec prob = calculateGenoFrq_arma(fts_mat, hap_pth.col(smp_idx(i)));
      prob(4) = prob(4) + prob(6);
      prob.shed_row(6);
      prob = prob % gen_lik.col(i);
      // prob = arma::normalise(prob, 1);
      arma::ucolvec elem = arma::linspace<arma::ucolvec>(0, 8, 9);
  ...
  arma::imat imp_smp = arma::zeros<arma::imat>(10, raw_smp.n_cols);
  imp_smp.row(0) = smp_gen;
  imp_smp.rows(1, 9) = smp_cnt;
...
arma::imat groupSample(const arma::imat& imp_smp, const int& evt_gen) {
  ...
  arma::imat smp_cnt = arma::zeros<arma::imat>(9, smp_gen.n_cols);
  arma::ucolvec grp_row = arma::linspace<arma::ucolvec>(1, 9, 9);
  for (arma::uword i = 0; i < smp_gen.n_elem; i++) {
  ...
  arma::imat grp_smp = arma::zeros<arma::imat>(11, smp_gen.n_cols);
  grp_smp.row(0) = smp_gen;
  grp_smp.row(1) = smp_siz;
  grp_smp.rows(2, 10) = smp_cnt;

  arma::icolvec evt_smp = arma::zeros<arma::icolvec>(11);
  evt_smp(0) = evt_gen;
...
double calculateLogGenoLikelihood_arma(const arma::dmat& raw_smp, const arma::imat& imp_smp) {
  ...
  arma::drowvec gen_lik = arma::sum(raw_smp.rows(1, 9) % imp_smp.rows(1, 9), 0);
...
// [[Rcpp::export]]
List runBPF_arma(const arma::dmat& sel_cof, const double& rec_rat, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::dmat& hap_pth, const arma::dmat& raw_smp, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  // ensure RNG gets set/reset
  RNGScope scope;

  arma::imat imp_smp = imputeSample(raw_smp, sel_cof, evt_gen, hap_pth);

  arma::imat grp_smp = groupSample(imp_smp, evt_gen);
  arma::irowvec smp_gen = grp_smp.row(0);
  arma::irowvec smp_siz = grp_smp.row(1);
  arma::imat smp_cnt = grp_smp.rows(2, 10);
  ...
  arma::imat gen_cnt_tmp = calculateGenoCnt_arma(smp_cnt.col(0));
  for (arma::uword i = 0; i < pcl_num; i++) {
    gen_frq_tmp.col(i) = calculateGenoFrq_arma(fts_mat, hap_frq_tmp.col(i));
    for (arma::uword j = 0; j < gen_cnt_tmp.n_cols; j++) {
      wght_tmp(i) = wght_tmp(i) + calculateMultinomProb_arma(gen_cnt_tmp.col(j), smp_siz(0), gen_frq_tmp.col(i));
    }
  }
  ...
  for (arma::uword k = 1; k < evt_ind; k++) {
    ...
    arma::imat gen_cnt_tmp = calculateGenoCnt_arma(smp_cnt.col(k));
    for (arma::uword i = 0; i < pcl_num; i++) {
      ...
      gen_frq_tmp.col(i) = calculateGenoFrq_arma(fts_mat, hap_frq_tmp.col(i));
      for (arma::uword j = 0; j < gen_cnt_tmp.n_cols; j++) {
        wght_tmp(i) = wght_tmp(i) + calculateMultinomProb_arma(gen_cnt_tmp.col(j), smp_siz(k), gen_frq_tmp.col(i));
      }
    }
  ...
  if (smp_gen(evt_ind) == smp_gen(evt_ind - 1)) {
    ...
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateWFD_arma(sel_cof.col(0), rec_rat, pop_siz.subvec(smp_gen(evt_ind - 1), smp_gen(evt_ind)), ref_siz, hap_frq_tmp.col(i), smp_gen(evt_ind - 1), smp_gen(evt_ind), ptn_num);
...
void calculateLogLikelihood_arma(double& log_lik, arma::dmat& frq_pth, const arma::dmat& sel_cof, const double& rec_rat, const arma::icolvec& pop_siz, const int& ref_siz, const arma::irowvec& smp_gen, const arma::irowvec& smp_siz, const arma::field<arma::imat>& ptl_cnt, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  if (smp_gen(evt_ind) != smp_gen(evt_ind - 1)) {
    for (arma::uword i = 0; i < pcl_num; i++) {
      arma::dmat path = simulateWFD_arma(sel_cof.col(0), rec_rat, pop_siz.subvec(smp_gen(evt_ind - 1), smp_gen(evt_ind)), ref_siz, hap_frq_pst.col(i), smp_gen(evt_ind - 1), smp_gen(evt_ind), ptn_num);
      hap_frq_pth.subcube(0, (smp_gen(evt_ind - 1) - smp_gen(0)) * ptn_num, i, 3, (smp_gen(evt_ind) - smp_gen(0)) * ptn_num, i) = path;
      hap_frq_pre.col(i) = arma::vectorise(path.tail_cols(1), 0);
    }
...
// [[Rcpp::export]]
List calculateOptimalParticleNum_arma(const arma::dmat& sel_cof, const double& rec_rat, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::dmat& hap_pth, const arma::dmat& raw_smp, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& gap_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
...
List runPMMH_arma(const arma::dmat& sel_cof, const double& rec_rat, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::dmat& raw_smp, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num) {
  ...
  arma::dcube frq_pth_chn = arma::zeros<arma::dcube>(4, arma::uword(max(raw_smp.row(0)) - min(raw_smp.row(0))) + 1, itn_num);
  arma::icube imp_smp_chn = arma::zeros<arma::icube>(9, raw_smp.n_cols, itn_num);
  ...
  frq_pth.fill(0.25);
  arma::imat imp_smp = imputeSample(raw_smp, sel_cof_chn.slice(0), evt_gen, frq_pth);
  imp_smp_chn.slice(0) = imp_smp.rows(1, 9);
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    imp_smp = imputeSample(raw_smp, sel_cof_chn.slice(i - 1), evt_gen, frq_pth_chn.slice(i - 1));
    imp_smp_chn.slice(i) = imp_smp.rows(1, 9);

    grp_smp = groupSample(imp_smp, evt_gen);
    smp_gen = grp_smp.row(0);
    smp_siz = grp_smp.row(1);
    smp_cnt = grp_smp.rows(2, 10);
    for (arma::uword k = 0; k < smp_gen.n_elem; k++) {
      ptl_cnt(k) = calculateGenoCnt_arma(smp_cnt.col(k));
    }
...
List runAdaptPMMH_arma(const arma::dmat& sel_cof, const double& rec_rat, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::dmat& raw_smp, const arma::uword& ptn_num, const arma::uword& pcl_num, const arma::uword& itn_num, const arma::drowvec& stp_siz, double& apt_rto) {
  ...
  for (arma::uword i = 1; i < itn_num; i++) {
    ...
    arma::imat imp_smp = imputeSample(raw_smp, sel_cof_chn.slice(i - 1), evt_gen, frq_pth_chn.slice(i - 1));
    imp_smp_chn.slice(i) = imp_smp.rows(1, 9);
