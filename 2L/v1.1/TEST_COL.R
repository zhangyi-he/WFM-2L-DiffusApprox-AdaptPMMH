...
# call R functions
source("./Code/Code v1.0/Code 2L/Code v1.1/RFUN_COL.R")
...
save(model, sel_cof, rec_rat, pop_siz, int_con, smp_gen, smp_siz, obs_hap, smp_cnt, smp_frq, pop_frq,
     file = "./Output/Output v1.0/TEST v1.1/TEST_COL_SimData.rda")

load("./Output/Output v1.0/TEST v1.1/TEST_COL_SimData.rda")

pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_SimData.pdf", width = 24, height = 18)
...
save(sel_cof, rec_rat, pop_siz, ref_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, BPF,
     file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BPF.rda")

load("./Output/Output v1.0/TEST v1.1/TEST_COL_BPF.rda")
...
pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BPF_Likelihood.pdf", width = 8, height = 6)
...
pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BPF_Particle.pdf", width = 72, height = 66)
...
save(sel_cof, rec_rat, pop_siz, ref_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, gap_num, OptNum,
     file = "./Output/Output v1.0/TEST v1.1/TEST_COL_OptNum.rda")

load("./Output/Output v1.0/TEST v1.1/TEST_COL_OptNum.rda")

opt_pcl_num <- OptNum$opt_pcl_num
log_lik_sdv <- OptNum$log_lik_sdv

pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_OptNum.pdf", width = 8, height = 6)
par(mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
...
save(sel_cof, rec_rat, pop_siz, ref_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num, sel_cof_chn,
     file = "./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH.rda")

load("./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH.rda")

pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH_Traceplot.pdf", width = 8, height = 12)
...
plot(1:itn_num, sel_cof_chn[1, 1:itn_num], type = 'l',
     ...
     main = "Trace plot for sel coeff of black")
abline(h = sel_cof[1], col = 'red', lty = 2, lwd = 2)

plot(1:itn_num, sel_cof_chn[2, 1:itn_num], type = 'l',
     xlab = "Iteration", ylab = "Selection coefficient",
     main = "Trace plot for sel coeff of chestnut")
abline(h = sel_cof[2], col = 'red', lty = 2, lwd = 2)
dev.off()

pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH_Autocorrplot.pdf", width = 16, height = 12)
...
autocorr.plot(sel_cof_chn[1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorrelation plot for sel coeff of black before thinning")
autocorr.plot(sel_cof_chn[2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorrelation plot for sel coeff of chestnut before thinning")

thn_num <- 8e+00
sel_cof_chn <- sel_cof_chn[, (1:round(dim(sel_cof_chn)[2] / thn_num)) * thn_num]
effectiveSize(as.mcmc(t(sel_cof_chn)))

autocorr.plot(sel_cof_chn[1, ], lag.max = 50, auto.layout = FALSE, main = "Autocorrelation plot for sel coeff of black after thinning")
autocorr.plot(sel_cof_chn[2, ], lag.max = 50, auto.layout = FALSE, main = "Autocorrelation plot for sel coeff of chestnut after thinning")
dev.off()
...
# pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH_Posterior.pdf", width = 8, height = 12)
# par(mfrow = c(2, 1), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
# hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
#      xlab = "Selection coefficient",
#      main = "Posterior for sel coeff of black")
...
#      xlab = "Selection coefficient",
#      main = "Posterior for sel coeff of chestnut")
...
pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_PMMH_Posterior.pdf", width = 16, height = 8)
par(mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
...
image(sel_cof_pdf, col = colorRampPalette(rev(brewer.pal(11, 'Spectral')))(32),
      ...
      main = "Joint posterior for sel coeff")
abline(v = sel_cof[1], col = 'red', lty = 2, lwd = 2)
abline(h = sel_cof[2], col = 'red', lty = 2, lwd = 2)
abline(v = sel_cof_est[1], col = 'black', lty = 2, lwd = 2)
...
hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
     xlab = "Selection coefficient",
     main = "Marginal posterior for sel coeff of black")
lines(density(sel_cof_chn[1, ]), lwd = 2, col = 'black')
abline(v = sel_cof[1], col = 'red', lty = 2, lwd = 2)
abline(v = sel_cof_est[1], col = 'black', lty = 2, lwd = 2)
abline(v = sel_cof_hpd[1, 1], col = 'blue', lty = 2, lwd = 2)
...
hist(sel_cof_chn[2, ], breaks = seq(min(sel_cof_chn[2, ]), max(sel_cof_chn[2, ]), length.out = 50), freq = FALSE,
     xlab = "Selection coefficient",
     main = "Marginal posterior for sel coeff of chestnut")
lines(density(sel_cof_chn[2, ]), lwd = 2, col = 'black')
abline(v = sel_cof[2], col = 'red', lty = 2, lwd = 2)
abline(v = sel_cof_est[2], col = 'black', lty = 2, lwd = 2)
abline(v = sel_cof_hpd[2, 1], col = 'blue', lty = 2, lwd = 2)
...
# pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_AdaptPMMH_Posterior.pdf", width = 8, height = 12)
# par(mfrow = c(2, 1), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
# hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
#      xlab = "Selection coefficient",
#      main = "Posterior for sel coeff of black")
...
save(sel_cof, rec_rat, pop_siz, ref_siz, smp_gen, smp_siz, smp_cnt, ptn_num, pcl_num, itn_num, brn_num, thn_num, adp_set, stp_siz, apt_rto, BayesianProcedure,
     file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BayesProc.rda")

load("./Output/Output v1.0/TEST v1.1/TEST_COL_BayesProc.rda")
...
# pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BayesProc_Posterior.pdf", width = 8, height = 12)
# par(mfrow = c(2, 1), mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
# hist(sel_cof_chn[1, ], breaks = seq(min(sel_cof_chn[1, ]), max(sel_cof_chn[1, ]), length.out = 50), freq = FALSE,
#      xlab = "Selection coefficient",
#      main = "Posterior for sel coeff of black")
...
pdf(file = "./Output/Output v1.0/TEST v1.1/TEST_COL_BayesProc_Posterior.pdf", width = 16, height = 8)
par(mar = c(5.5, 5, 5.5, 2.5), cex.main = 1.75, cex.sub = 1.5, cex.axis = 1.5, cex.lab = 1.5)
layout(matrix(c(1, 1, 2, 3), nrow = 2, ncol = 2))
grd_num <- 1e+03
sel_cof_pdf <- kde2d(sel_cof_chn[1, ], sel_cof_chn[2, ], n = grd_num)
